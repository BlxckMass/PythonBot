...
@Component
public class TopCoinsCommand extends VkCommand {
    ...
    @Autowired
    public TopCoinsCommand(VkApiClient vk, GroupActor group, Logger log, UserService userService, PythonKeyboard pythonKeyboard) {
        super(vk, group, log);
        this.log = log;
    ...
    public void run(Message message) {
        ...
        int numberToper = 1;
        List<User> topUsers = userService.findUsersByTopBalance();
        StringBuilder topMessage = new StringBuilder("Топ по коинам:\n");

        for (User topUser : topUsers) {
            topMessage.append("♯" + numberToper +
                    " @id" + topUser.getId() + "(" + topUser.getNickname() + ")" +
                    " ⇨ " + topUser.getBalance() + " байтов\n");
            numberToper++;
        }
